...
def unet(input_size = (512,512,3),pretrained_weights = None):
...
#        recall = true_positives / (possible_positives + keras.epsilon())
        ...
        # Adaptation of the "round()" used before to get the predictions. Clipping to make sure that the predicted raw values are between 0 and 1.
        y_pred = keras.cast(keras.greater(keras.clip(y_pred, 0, 1), 0.5), keras.floatx())
        # Compute the number of true positives. Rounding in prevention to make sure we have an integer.
        true_positives = keras.round(keras.sum(keras.clip(y_true * y_pred, 0, 1)))
        # Compute the number of positive targets.
        possible_positives = keras.sum(keras.clip(y_true, 0, 1))
...
#        recall = recall_m(y_true, y_pred)
        ...
        recall_value = recall_m(y_true, y_pred)
    ...
    model.compile(optimizer = 'sgd', loss=[weighted_bce_dice_loss], metrics = [f1_m,recall_threshold(0.5),precision_threshold(0.5),'accuracy'])
